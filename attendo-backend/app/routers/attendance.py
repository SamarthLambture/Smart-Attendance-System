...
#     roll_number: str
#     student_name: str
#     dates: dict  # {date: status} where status is "P" (present) or "A" (absent)
#     total_present: int
#     total_classes: int
#     attendance_percentage: float
...
#     subject_name: str
#     subject_code: str
#     dates: List[str]  # All class dates
...
#     CRITICAL FIX: Prevents duplicate sessions from being created within short time window
#     This handles React StrictMode double-calls and accidental rapid clicks
#     """
...
#     # CRITICAL FIX: Check for ANY session created in the last 10 seconds
#     # This prevents duplicate sessions from React StrictMode or rapid clicks
...
#         # Found a session created within last 10 seconds - this is likely a duplicate call
#         # Just return the existing session with a new code
#         print(f"⚠️  Duplicate call detected! Reusing session {very_recent_session.id}")
...
#     # Check if there's an existing ACTIVE session (not expired yet)
#     existing_active_session = db.query(AttendanceSession).filter(
#         AttendanceSession.subject_id == request.subject_id,
#         AttendanceSession.is_active == True,
#         AttendanceSession.expires_at > current_time
#     ).first()
    
#     if existing_active_session:
#         # Active session exists - refresh it
#         print(f"♻️  Refreshing active session {existing_active_session.id}")
        
#         new_code = generate_session_code()
#         while db.query(AttendanceSession).filter(
#             AttendanceSession.session_code == new_code,
#             AttendanceSession.id != existing_active_session.id
#         ).first():
#             new_code = generate_session_code()
        
#         existing_active_session.session_code = new_code
#         existing_active_session.qr_data = new_code
#         existing_active_session.expires_at = current_time + timedelta(minutes=4)
        
#         db.commit()
#         db.refresh(existing_active_session)
        
#         return SessionResponse(
#             session_id=existing_active_session.id,
#             session_code=existing_active_session.session_code,
#             qr_data=existing_active_session.session_code,
#             expires_at=existing_active_session.expires_at,
#             subject_name=subject.subject_name,
#             subject_code=subject.subject_code
#         )
    
#     # No recent or active session - create a NEW one
#     # First, mark all old sessions as inactive
#     db.query(AttendanceSession).filter(
#         AttendanceSession.subject_id == request.subject_id,
#         AttendanceSession.is_active == True
#     ).update({"is_active": False})
    
#     db.commit()  # Commit the deactivation first
...
#         is_active=True,
#         date=current_time  # Explicitly set the date
#     )
...
#     print(f"✅ Created NEW session {new_session.id} for subject {subject.subject_code}")
...
# ):
#     """
#     Mark attendance for a student with photo verification
#     """
...
#     # Get total classes for this subject (all past sessions)
#     total_classes = db.query(AttendanceSession).filter(
#         AttendanceSession.subject_id == subject_id,
#         AttendanceSession.date < datetime.utcnow()  # Only past sessions
#     ).count()
...
#     Shows all students and all class dates in tabular format
#     """
...
#     # Get all sessions for this subject (sorted by date)
...
#             logger.info(f"ðŸ—'ï¸  Deleted photo: {photo_path}")
...
# ):
#     """
#     ðŸ†• Mark attendance with face verification
...
#         match_found, matched_student, distance = face_service.verify_face(
#             photo_path=str(photo_path),
...
#             logger.warning(f"âš ï¸  Face verification failed for {student.roll_number}")
#             raise HTTPException(
#                 status_code=403,
#                 detail="Face verification failed. Photo does not match registered face. Please try again or contact administrator."
#             )
        
#         # Check if matched student is the same as claimed student
#         if matched_student['email'] != student_email:
#             logger.error(f"âŒ Face mismatch! Photo from {student_email} matched {matched_student['email']}")
#             raise HTTPException(
#                 status_code=403,
#                 detail=f"Face verification failed. Face matches {matched_student['name']} ({matched_student['roll_number']}), not your registered face."
#             )
        
#         logger.info(f"âœ… Face verified for {student.name} ({student.roll_number}) - Distance: {distance:.4f}")
...
#     # ðŸ—'ï¸ Delete photo in background (verified and recorded)
...
#         "record_id": attendance_record.id,
#         "verified_distance": round(distance, 4)
#     }
...
async def mark_attendance(
    ...
    try:
        ...
        match_found, matched_student, similarity = face_service.verify_face(
            photo_path=str(photo_path),
            threshold=0.75  # YOUR threshold from face_test.py
        )
