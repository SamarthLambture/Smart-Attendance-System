...
async def register_student(
    ...
    temp_filename = f"{roll_number}_temp{file_extension}"
    temp_file_path = TEMP_PHOTO_DIR / temp_filename
    
    with open(temp_file_path, "wb") as buffer:
        shutil.copyfileobj(photo.file, buffer)
    
    # Move to permanent storage after OTP verification
    # For now, store in temp and will move after OTP verification
